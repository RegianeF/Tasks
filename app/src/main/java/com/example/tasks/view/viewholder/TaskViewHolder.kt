...
class TaskViewHolder(itemView: View, val listener: TaskListener) :
    ...
    private val mDateFormat = SimpleDateFormat("dd/MM/yyyy", Locale.ROOT)
    ...
    fun bindData(task: TaskModel) {
        ...
        //converter o formato pra uma data pra depois pode usar
        val date = SimpleDateFormat("yyyy-MM-dd").parse(task.dueDate)
        this.mTextDueDate.text = mDateFormat.format(date)

        if (task.complete) {
            mTextDescription.setTextColor(Color.GRAY) //mudando a cor
